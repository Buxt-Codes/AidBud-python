...
class llm:
    def __init__(self, model_id: str = "google/gemma-3n-E2B-it", config: Config = Config()):
        ...
        self.max_workers = config.llm["max_workers"] if config.llm["max_workers"] is not None else os.cpu_count()
    ...
    def generate(
        ...
        messages = self._prepare_prompt(prompt, images=images, audios=audios)
        ...
        original_prompt_length = len(self.processor.batch_decode(inputs)[0])
        return response_text[0][original_prompt_length:].strip()